...
var s_hue,l_hue, s_sat, l_sat, s_bri, l_bri, s_opacity, l_opacity, s_brush_size, l_brush_size
...
function getData(xpos, ypos ,hue,sat,bri,op,brush_size){
   var data = {
    x: xpos,
    y: ypos, 
    h: hue,
...
function mouseDragged() {
 if( !(mouseY>windowHeight-80 && mouseX< 800 )){
    // créer l'objet data avec les paramètres
   var size = (abs(pmouseX-mouseX)+abs(pmouseY-mouseY)) 
    var data = getData(mouseX,mouseY,s_hue.value(),s_sat.value(),s_bri.value(),s_opacity.value(),size/2 +s_brush_size.value())
    draw_params(data)
...
function draw_params(data){
  push()
  noStroke()
  fill(data.h, data.s, data.b, data.o) // on applique la teinte et on garde une transparence importante
  translate(data.x,data.y)
  //ellipse(0,0,40,40)
  for (var i = 0 ; i < int(data.siz)*2 ; i = i+1){
    // coordonées polaire = rayon + angle
    var radius = random(data.siz) 
    ...
    ellipse(xpos, ypos, map(radius,0,data.siz,data.siz/6,0), map(radius,0,data.siz,data.siz/6,0) )
  }
  pop()
}
